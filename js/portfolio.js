/* portfolio.js — Portfolio-specific: typed roles, stagger, sidebar nav, contact form */
(function () {

  /* ── EMAILJS INIT ── */
  try { emailjs.init('nHMmaEeDsQ6yN-2Kx'); } catch (e) {}

  /* ── EXTEND CURSOR HOVER to portfolio card elements ── */
  /* app.js covers a, button, input, textarea — add portfolio-specific interactives */
  var ring = document.getElementById('cursor-ring');
  if (ring) {
    document.querySelectorAll('.project-card, .skill-card, .cert-item, .exp-card').forEach(function (el) {
  ...
  var mobileLinks  = document.querySelectorAll('.portfolio-mobile-nav a');
  function setActive() {
    var y = window.scrollY + 160, cur = '';
    sections.forEach(function (s) { if (s.offsetTop <= y) cur = s.id; });
    var href = '#' + cur;
    sidebarLinks.forEach(function (a) { a.classList.toggle('active', a.getAttribute('href') === href); });
    mobileLinks.forEach(function (a)  { a.classList.toggle('active', a.getAttribute('href') === href); });
  }
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      emailjs.sendForm('service_0cgjbdf', 'template_8stcqsc', form)
        .then(function () {
          setStatus('Message sent - I will be in touch soon.', false);
          form.reset();
